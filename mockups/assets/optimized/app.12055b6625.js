  ...
  document.addEventListener('keydown', function(e) {
    if (e.key === 'Escape') cerrarModal();
	  ...
	  const CHECKOUT_COUNTRIES = [
	    { code: 'CO', name: 'Colombia', shippingUsd: 0 },
	    { code: 'US', name: 'Estados Unidos', shippingUsd: 130 },
	    { code: 'MX', name: 'México', shippingUsd: 130 },
	    { code: 'CA', name: 'Canadá', shippingUsd: 140 },
	    { code: 'ES', name: 'España', shippingUsd: 230 },
	    { code: 'FR', name: 'Francia', shippingUsd: 230 },
	    { code: 'DE', name: 'Alemania', shippingUsd: 230 },
	    { code: 'IT', name: 'Italia', shippingUsd: 230 },
	    { code: 'NL', name: 'Países Bajos', shippingUsd: 230 },
	    { code: 'CL', name: 'Chile', shippingUsd: 120 },
	    { code: 'PE', name: 'Perú', shippingUsd: 120 },
	    { code: 'EC', name: 'Ecuador', shippingUsd: 115 },
	    { code: 'PA', name: 'Panamá', shippingUsd: 115 }
	  ];
  ...
  function leerUltimoCheckout() {
    try {
	  ...
	  function actualizarModoCheckout(slug, options = {}) {
	    ...
	    if (note) {
	      if (isInternational) {
	        const shippingCop = country.shippingUsd * CHECKOUT_USD_TO_COP_REFERENCE;
	        const totalCop = (product.priceCop || 0) + shippingCop;
	        note.innerHTML = [
	          '<strong>' + escapeHtml(t('checkout.internationalNoteTitle')) + '</strong>',
	          '<span>' + escapeHtml(
	            t('checkout.internationalNote')
	              .replace('{country}', country.name)
	              .replace('{price}', formatearCopCheckout(product.priceCop || 0))
	              .replace('{usd}', 'USD ' + country.shippingUsd)
	              .replace('{cop}', formatearCopCheckout(shippingCop))
	        ...
	        if (options.track) {
	          trackSimio('shipping_cost_viewed', product.slug, {
	            ...
	            shipping_usd: country.shippingUsd
	          });
	        }
	      } else {
	        note.innerHTML = [
  ...
  const I18N = {
    es: {
      ...
      'checkout.countryLabel': 'País',
	      ...
	      'checkout.postalRequired': 'El código postal es necesario para cotizar envíos internacionales.',
	      ...
	      'checkout.internationalNoteTitle': 'Envío internacional',
	      'checkout.internationalNote': '{country}: pieza {price} + envío DHL ref. {usd} ({cop}) = total preliminar {total}. Confirmamos el total definitivo por email antes de cobrar.',
	      'checkout.internationalDdu': 'No incluye aranceles, impuestos ni gastos de importación en destino.',
    ...
    en: {
      ...
      'checkout.countryLabel': 'Country',
	      ...
	      'checkout.postalRequired': 'Postal code is required for international shipping quotes.',
	      ...
	      'checkout.internationalNoteTitle': 'International shipping',
	      'checkout.internationalNote': '{country}: piece {price} + DHL shipping ref. {usd} ({cop}) = preliminary total {total}. We confirm the final total by email before charging.',
	      'checkout.internationalDdu': 'Import duties, taxes, or destination fees are not included.',
