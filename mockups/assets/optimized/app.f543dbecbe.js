  ...
  async function generarPreviewEncargo(root) {
    ...
    const tareaPendienteExistente = Boolean(encargoState.tripoTaskId && tripoEstaPendiente(encargoState.tripoStatus));

    if (!SONDEO_EMAIL_RE.test(email)) {
      setEncargoStatus(root, 2, 'Vuelve al paso 1 y confirma tu email.', true);
      return;
    }
    if (!file && !tareaPendienteExistente) {
    ...
    try {
      if (!encargoState.imageDataUrl && file) {
        encargoState.imageDataUrl = await comprimirImagenEncargo(file);
        setFrameImage(root.querySelector('#encargo-upload-preview'), encargoState.imageDataUrl, 'Imagen subida');
      } else if (!encargoState.imageDataUrl && !tareaPendienteExistente) {
        throw new Error('Sube una imagen antes de generar el modelo 3D.');
      }
  ...
  async function enviarSolicitudEncargo(root) {
    ...
    const observaciones = [
      modelNotes ? `Ajustes sobre modelo generado: ${modelNotes}` : '',
      finalObservaciones
    ].filter(Boolean).join('\n\n').slice(0, 900);
    const payload = {
